...
public interface AcmeProfileController extends AuthProtectedController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "ACME Profile details retrieved"),
            @ApiResponse(responseCode = "404", description = "ACME Profile not found",
                    content = @Content(schema = @Schema(implementation = ErrorMessageDto.class)))})
    @GetMapping(path = "/{uuid}", produces = {"application/json"})
    AcmeProfileDto getAcmeProfile(@Parameter(description = "ACME Profile UUID") @PathVariable String uuid)
            throws NotFoundException;

    @Operation(summary = "Create ACME Profile")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "ACME Profile created"),
            @ApiResponse(responseCode = "404", description = "RA Profile not found",
                    content = @Content(schema = @Schema(implementation = ErrorMessageDto.class)))})
    @PostMapping(consumes = {"application/json"}, produces = {"application/json"})
    ResponseEntity<UuidDto> createAcmeProfile(@RequestBody @Valid AcmeProfileRequestDto request)
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "ACME Profile updated"),
            @ApiResponse(responseCode = "404", description = "ACME Profile or RA profile not found",
                    content = @Content(schema = @Schema(implementation = ErrorMessageDto.class)))})
    @PutMapping(path = "/{uuid}", consumes = {"application/json"}, produces = {"application/json"})
    AcmeProfileDto editAcmeProfile(@Parameter(description = "ACME Profile UUID") @PathVariable String uuid,
            @RequestBody @Valid AcmeProfileEditRequestDto request)
            throws ConnectorException, AttributeException, NotFoundException;

    @Operation(summary = "Delete ACME Profile")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "ACME Profile deleted"),
            @ApiResponse(responseCode = "404", description = "ACME Profile not found",
                    content = @Content(schema = @Schema(implementation = ErrorMessageDto.class)))})
    @DeleteMapping(path = "/{uuid}", produces = {"application/json"})
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void deleteAcmeProfile(@Parameter(description = "ACME Profile UUID") @PathVariable String uuid)
            throws NotFoundException, ValidationException;

    @Operation(summary = "Enable ACME Profile")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "ACME Profile enabled"),
            @ApiResponse(responseCode = "404", description = "ACME Profile not found",
                    content = @Content(schema = @Schema(implementation = ErrorMessageDto.class)))})
    @PatchMapping(path = "/{uuid}/enable", produces = {"application/json"})
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void enableAcmeProfile(@Parameter(description = "ACME Profile UUID") @PathVariable String uuid)
            throws NotFoundException;

    @Operation(summary = "Disable ACME Profile")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "ACME Profile disabled"),
            @ApiResponse(responseCode = "404", description = "ACME Profile not found",
                    content = @Content(schema = @Schema(implementation = ErrorMessageDto.class)))})
    @PatchMapping(path = "/{uuid}/disable", produces = {"application/json"})
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void disableAcmeProfile(@Parameter(description = "ACME Profile UUID") @PathVariable String uuid)
            throws NotFoundException;
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void bulkEnableAcmeProfile(@io.swagger.v3.oas.annotations.parameters.RequestBody(description = "ACME Profile UUIDs",
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void updateRaProfile(@Parameter(description = "ACME Profile UUID") @PathVariable String uuid,
